...
mod tests {
    ...
    fn to_body_maps_each_seed_kind() {
        assert_eq!(
            to_body("artist:44042").unwrap(),
            serde_json::json!({"artist_id": 44042})
        );
        assert_eq!(
            to_body("track:9").unwrap(),
            serde_json::json!({"track_id": 9})
        );
        assert_eq!(
            to_body("album:abc").unwrap(),
            serde_json::json!({"album_id": "abc"})
        );
